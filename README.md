...
Consider the following binary matrix (with added column and row counts in bold)
...
I'm unsure now why I took it upon myself to handle these validations as these were sanity constraints already imposed by the Problem creators.
...
### Version 7
I cleaned up my code in this version. Instead of transposing and re-processing the matrix conditions inside my **preliminaryValidation** function, I cleaned it up to just process from a rows-perspective. Then I transposed the matrix and invoked the same function again. This fixed whatever glitches were there in my previous "unmanageable" version. By now, I was at my wit's end and was more that willing to throw in the towel if this didn't work.

Submission Outcome: 40 points (Arghh! but at least 40 > 0) The only failures were due to TLEs. 

### Version 8
At this point I had no idea how to further refine my logic. So I felt that I was missing something very basic and probably the overall logic itself was flawed (or at least an overkill). I was actually considering writing a program to churn out various combinations of random inputs (as test-cases). My "plan" was to solve these on paper manually and then let my code solve the same. I would then try to figure out if my logic was flawed or alternatively I might discover some other pattern that I could exploit to find a faster solution.

Thankfully, I had a kid visiting me at home at that time and I asked her to come up with a few different random test-cases manually for me. Of course I had to explain to this college kid what a matrix was. Let's observe 1 minute of silence for our education system. Moving on.
...
Also, I optimized the comparison by first sorting the row/column summations list and then picking the max and min values from just the ends. Additionally I did the sum(row-summations) == sum(column-summations).
